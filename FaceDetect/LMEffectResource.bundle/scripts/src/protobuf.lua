...
function pack(pattern, ...)
    --加入头信息
    local index = string.find(pattern, "protoID")
    if index then
        return nil
    end

    index = string.find(pattern, "protoVer")
    if index then
        return nil
    end

    pattern = pattern .. " protoID protoVer";

	local pat = _pattern_cache[pattern]
    return c._pattern_pack(pat.CObj, pat.format, pat.size , 2, ...)
